 ...
 * User: tcostazno
 */
public class B2bmbFTPProducer extends RemoteFileProducer<FTPFile> {
    private static final transient Logger LOG = LoggerFactory.getLogger(B2bmbFTPProducer.class);
    ...
    public String createFileName(Exchange exchange) {
        ...
        String directoryName = "localOutDirectoryName";
        if(getEndpoint().getB2bDirectoryLocation() != null && !"".equals(getEndpoint().getB2bDirectoryLocation())) {
            directoryName = getEndpoint().getB2bDirectoryLocation();
        }
        ...
        parentAttribute.setRefName(directoryName);
        dynamicSearchRequest.getSearchFields().getAttributes().put(directoryName, parentAttribute);
    ...
    private FtpConfiguration buildFtpConfiguration(CommunicationConfiguration cc) {
        ...
        ftpConfiguration.setDirectory(cc.getInDirectoryName());
